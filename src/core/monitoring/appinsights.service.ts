...
class AppInsightsService {
  ...
  private nodeEnv = 'production';
  private cloudRole = process.env.APPINSIGHTS_ROLE_NAME;
  ...
  public appInsightsConfig(): void {
    try {
      if (
        ...
        !isUndefined(process.env.APPINSIGHTS_INSTRUMENTATIONKEY)
      ) {
        this.appInsights
  ...
  public appInsightsRoleConfig(): void {
    if (!isNull(this.cloudRole) && !isUndefined(this.cloudRole)) {
      this.appInsights.defaultClient.context.tags[
        this.appInsights.defaultClient.context.keys.cloudRole
      ] = this.cloudRole;
    }
  }
